...
class SeedViews(APIView):
    ...
    def get(self, request):
        try:
            result = self.__mongoHelper.getCollection("seed").find()
    ...
    def post(self, request):
        try:
            ...
            if (
                ...
                result = list(self.__mongoHelper.getCollection("seed").find().sort({"$natural": -1}).limit(1))[0]
                request_body["_id"] = result["_id"] + 1
                self.__mongoHelper.getCollection("seed").insert_one(request_body)
    ...
    def put(self, request):
        try:
            ...
            if (
                ...
                self.__mongoHelper.getCollection("seed").update_one({ "_id": request_body["_id"] }, { "$set": request_body })
    ...
    def delete(self, request):
        try:
            ...
            if id != None:
                self.__mongoHelper.getCollection("seed").delete_one({ "_id" : id })
